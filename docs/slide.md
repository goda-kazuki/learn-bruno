...
- クラウド同期前提 → データが外部に渡すことへの懸念、 Git 管理しづらい
...
- `environments` ディレクトリでシークレット変数を管理
...
1. **宣言的テスト** — `assert` ブロックで簡潔に書ける
1. **オフライン・無料** — クラウド依存なし
2. **CI/CD 対応** — CLI でパイプラインに組み込める
3. **移行が容易** — Postman コレクションをインポート可能
